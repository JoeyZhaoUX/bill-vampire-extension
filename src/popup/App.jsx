...
// Mirror of App.jsx#importSubsFromExtensionHash: hash key `ext_subs`, base64
// of JSON array { name, price, currency, cycle, category, nextChargeAt }.
function buildVerdictUrl(findings) {
  if (!findings?.length) return `${WEB_APP}?import=ext`;
  const payload = findings.map(f => ({
    name: f.name,
    price: String(Number.isFinite(f.amountUsd) ? f.amountUsd : 0),
    currency: 'USD',
    cycle: f.cycle === 'yearly' ? 'yearly' : 'monthly',
    category: 'Other',
    nextChargeAt: f.nextChargeAt || null,
  }));
  const json = JSON.stringify(payload);
  const encoded = encodeURIComponent(btoa(unescape(encodeURIComponent(json))));
  return `${WEB_APP}?import=ext#ext_subs=${encoded}`;
}
  ...
  return (
    <div style={{ padding: 16 }}>
      <header style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <div style={{ width: 28, height: 28, borderRadius: 8, background: 'linear-gradient(135deg,#7c3aed,#f43f5e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>🧛</div>
        <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        ...
        <button onClick={() => openWebApp(buildVerdictUrl(findings))}
          disabled={findings.length === 0}
          style={{ flex: 1, padding: '8px 10px', background: '#1C1C2A', color: '#e2e8f0', border: '1px solid rgba(100,116,139,.3)', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: findings.length ? 'pointer' : 'not-allowed', opacity: findings.length ? 1 : 0.5 }}>
          Open full verdict
        </button>
      </div>
